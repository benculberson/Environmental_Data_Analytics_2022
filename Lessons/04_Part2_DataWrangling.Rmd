...
# Format sampledate as date, always gotta do this-needs to be a habit
...
```{r}
# Gather nutrient data into one column (the outdated version)
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #both ends of that slice are inclusive
#the columns from tn_ug to po4 should be pushed into just two long columns
#nutrient will tell you what kind of nutrient it is (rather than many nutrient columns)
#concentration will tell you what concentration it is (regardless of the nutrient)
...
#there is a lot of NAs in the concentration column, so we need to get rid of them since we only care about the concentration value of that specific nutrient at that specific date
...
# Spread nutrient data into separate columns (this is the outdated version)
...
LitterFiles#the above function is fucking cool, will be needed for reading in multiple csvs
