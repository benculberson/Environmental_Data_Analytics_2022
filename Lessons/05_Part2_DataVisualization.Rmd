...
O3plot4 <- ggplot(EPAair) +
  ...
  theme_classic()
print(O3plot4)
#ok these adjustments just kind of changed the background a litte
```
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  ...
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold", color = "green") +   # hjust = 1 means left align
  geom_text(x = as.Date("2020-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold", color = "orange") +
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold", color = "red") +
...
NvsP2 <-
  ...
  labs(title ="Let's get wasted", x="Concentration of tp_ug", y = "Concentration of tn_ug", color = "Depth", shape = "Lake Name") + # change your legend labels here, notice how to change the legend titles, I need to call the color and shape sections
...
#I guess the plot_grid function does what we want huh

```

### Saving plots <-crucial for assignments I think
...
ggplot(Neonics) +
  reorder(Neonics$Endpoint, count, FUN = length) +
  geom_bar(aes(x = Endpoint))
#yeah im not gonna do this, hopefully she goes over it in class
