...
epa_pm25_sites_sf <- read_csv('../Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% 
  ...
  summarize(
    meanPM = mean(`Daily Mean PM2.5 Concentration`,na.rm=T), #remember, we're grouping by the above things and getting their means (he describes it as collapsing it by site)
    maxPM = max(`Daily Mean PM2.5 Concentration`,na.rm=T)
    ) %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #this creates the spatial dataframe (spatial features objects)

#Inspect the object
class(epa_pm25_sites_sf)

#What is its CRS again?
st_crs(epa_pm25_sites_sf)$epsg #this calls the epsg CRS value
...
mapView(counties_sf, zcol = "ALAND") #changing the attribute to say AWATER Yields a different map
```
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #FIPS is number but we're looking at it as a factor, if we brought them in as numbers, when we combined the STATEFP and the COUNTYFP, the resulting number would'nt make sense, we need to keep that 0 in there
...
#Tidyverse version of the join, same thing as above but using the tidyverse package
...
#so basically, we grouped by country, then summed them into a single state
