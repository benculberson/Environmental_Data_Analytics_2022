...
* **Data Frames** 2 dimensional structure that is more general than a matrix. Columns can have different modes (e.g., numeric and factor). When we import csv files into the R workspace, they will enter as data frames. (remember, dfs need to have columns of all the same length)
...
matrix1[4, ] #row indexed first, column indexed second
matrix1[ , 3] #
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
> Answer: We have data, values, and functions 

Question: In the R chunk below, write "dataframe1$". Press `tab` after you type the dollar sign. What happens?

> Answer: R gives us a series of column names possibilities, we can now choose between the names
...
```{r}
high_temperature <- c(49, 58, 39, 41, 44, 37, 41, 51, 52, 53)
low_temperature <- c(30, 30, 18, 26, 26, 19, 26, 26, 28, 39)
...
C_high_temperature <-conversion(high_temperature)
C_low_temperature <- conversion(low_temperature)
...
```{r}
DurhamTemps <- data.frame("HighF" = high_temperature, "LowF" = low_lemperature, "HighC" = signif(C_high_temperature, digits = 3), "LowC" = signif(C_low_temperature, digits = 3))


#round() could also work here

data.frame()
#This is how to change something to a data.frame!!!
...
```{r}
summary(DurhamTemps)
sd(DurhamTemps$HighF) #looks like the sd() function does not accept a df, only a vector
sd(DurhamTemps$LowF)
sd(DurhamTemps$HighC)
sd(DurhamTemps$LowC)
#help(sd)
```
...
today <- Sys.Date()
<<<<<<< HEAD
format(today, format = "%B %d, %Y") #works with stuff in between the date formatting selection, like I can put in slashes, commas, or hyphens
...
#Since the format is year-month-day we will use function ymd(), this function takes the character of the date (remember it gets read in as a char) and returns it as a date object-the best part: it doesn't matter the formatting of the date. As long as we get the order mdy (or what have you) in the right order, it will work!!!
...
fix.early.dates <- function(d, cutoff) {
       ...
       year(d) <- ifelse(m > cutoff, 1900+m, 2000+m)  #this will update year(d), year() is a function that returns the year for a data object, if the date is beyond the cutoff, it's in the 1900s, if it's befpre the cutoff, it's in the 2000s
       return(d)
}
