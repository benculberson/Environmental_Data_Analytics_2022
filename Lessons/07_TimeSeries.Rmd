...
#notice the time series object only takes on column of data
```
...
EnoDischarge.late_Decomposed <- stl(EnoDischarge.late_ts, s.window = "periodic") #I guess we chose a periodic seasonal extraction, we could have done weekly or something higher, I think we chose periodic because we're not sure of the time repitition

# Visualize the decomposed series. 
plot(EnoDischarge.late_Decomposed)

# We can extract the components and turn them into data frames
EnoDischarge.late_Components <- as.data.frame(EnoDischarge.late_Decomposed$time.series[,1:3]) #$time.series[,1:3] is how you call the 3 decomposed plots
EnoDischarge.late_Components #this is the seasonal, trend, remainder columns 
...
#ok we're adding in two components from the original data set, the observed value and the date
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) + #this line is designed to illustrate the seasonality is below 0 sometimes (the lty is the dotted line)
  ylab(expression("Discharge (ft"^3*"/s)"))
#Aaand now we can see the seasonality put on top of the data observations
#THIS is what she was talking about above - the seasonality goes into the negative when we know that it should be bounded
...
Two types of trends may be present in our time series dataset: **monotonic/deterministic** or **stochastic**. Monotonic trends are a gradual shift over time that is consistent in direction, constant over time, for example in response to land use change. Stochastic means random, the slope of the trend changes over time.
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #zoo::na.approx(AWND)-this names the package you want to use to be specific (we also don't need to library the package), the zoo package and na.approx function does the linear interpolation (maybe ask Luana about this?)
...
Specific tests for monotonic trend analysis are listed below, with assumptions and tips: (these functions determine if you ts set has a trend or is stationary, H0 is stationary for all of these tests)
...
* **Spearman Rho**: no seasonality, non-parametric, missing data allowed. Function: `cor.test(method="spearman")` (package: stats) - the null is again: no monotonic relationship
...
The null is that there is no stochastic trend

WE WILL LIKELY USE THE PACKAGES ABOVE IN THE LAB/ASSIGNMENT
...
f_month <- month(first(wind_data_clean$DATE)) #first() calls the first observation of the column
f_year <- year(first(wind_data_clean$DATE)) #same thing, but first year not-first month
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month), #notice we use the f_month and f_year here to start off the time series 
                   frequency=12) #12 = monthly data

#decompose
wind_data_decomp <- stl(wind_data_ts,s.window = "periodic")
plot(wind_data_decomp)
#we now know we have a seasonal data set
...
#Hypothesis for the Mann-Kendall?
#M0: the data is stationary
#M1: the data is not stationary, there is a trend

wind_data_trend2 <- trend::smk.test(wind_data_ts) #this is another way to run the Mann-Kendall stationarity test (with a different package)
# Inspect results
wind_data_trend2
summary(wind_data_trend2)
#this test shows the breakdown of the effect each season has (as well as their respective statistical significance)
...
> Answer: WE definitely have a decreasing trend of average wind speed over time.
