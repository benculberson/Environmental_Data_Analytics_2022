...
* Requests are often sent as browser URLs. They usually include:
 ...
 Address of the server: www.google.com
 Service Name: search
 Parameters: q = NSOE
...
We used to have Neuse River Site Near Clayton, now we have Eno River Near Durham, NC
...
#we've set the site number, the period, and the format we want to save our data in the URL above

#Get the data, which starts on line 30
gage_data <- read.table(theURL,skip = 28,header=TRUE,sep='\t') 
#There are 28 header lines in this above URL so we'll skip them (we did have to count them), there is a header on the columns so we'll skip them too, and then the data is separated by tabs
...
#we now have the scraped data tidied up for our purposes
...
#we've set the site number, the period, and the format we want to save our data in the URL above
#to do the exercise, note that I just changed the site_no and the period 

#Get the data, which starts on line 3
gage_data <- read.table(theURL,skip = 28,header=TRUE,sep='\t') 
#There are 28 header lines in this above URL so we'll skip them (we did have to count them), there is a header on the columns so we'll skip them too, and then the data is separated by tabs
...
pcode = '00060' #discharge (cfs) <- this code gets us cubic feet per second

#Identify statistic code for daily values: 
scode = "00003"  #mean, that's what this code gets us
...
```{r}
#Apply the API key
#census_api_key("91573873ef61ecb717d7ee21c60d0ca8aed3807b", install = TRUE)
...
scrape.it <- function(the_year){
  ...
  my_df <-data.frame(
    "State" = the_states,
    "Price" = as.numeric(the_price),
    "Capacity" = as.numeric(gsub(",","",the_capacity)), #the gsub function handles the comma in the character 
    "Generation" = as.numeric(gsub(",","",net_generation)),
    "Retail Value" = as.numeric(gsub(",","",total_retail)),
    year = the_year
  )
