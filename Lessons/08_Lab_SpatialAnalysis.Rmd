...
#my original way, but it looks like we're using ggplot to do this
```
...
#mapview(huc8_sf, fill = 'orange') + 
#  mapview(upperNeuse_sf, fill = 'purple')
...
wind_data <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv", colClasses = c("HUC_12" = 'factor')) %>% #this colClasses function ensures that we don't treat the HUC_12 values as numbers but rather classes so that the 0 in front stays there
  mutate(HUC_8 = substr(HUC_12,0,8)) %>% #(X, start, stop)
  group_by(HUC_8) %>%
  summarize(Energy = sum(AvgWindEnergy))


#Join to HUC_8 features
HUC8_sf_join <-  merge(x = huc8_sf,
                           y = wind_data, 
                           by.x = "HUC_8", by.y = "HUC_8")
...
epa_sf_utm <-      st_transform(epa_pm25_sites_sf, crs = 26917) #it appears that we haven't read these in for the lab document, if you copy this code to the part2 document, it should work
counties_sf_utm <- st_transform(counties_sf_utm, crs = 26917)
state_sf_utm <-    st_transform(state_sf_utm, crs = 26917)
huc8_sf_utm <-     st_transform(huc8_sf_utm, crs = 26917)
...
#we've now eliminated all the area outside of that triangle area
```
