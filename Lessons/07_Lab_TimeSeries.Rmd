...
Rainfall_BR_processed <-
  Rainfall_BR %>% 
  rename( Rainfall = Rainfall....MM.) %>% #notice the renaming
  separate(Statistics,c("Null","Month","Null2")," ") %>% #We're separating the statistics column into 3 columns named Null, Month, Null2 (the reason we're creating the 2 null columns is because there's a space before the month and the word "average is after the month) the " " means that we will use the spaces as separators
  mutate( Date = my(paste0(Month,"-",Year))) %>% #so now we have a Month-Year column, but it does not appear to be a date object?
...
#we want to be sure that the join happens at the exact same month to month

class(BRA_complete$Date)
class(Rainfall_BR_processed$Date)
...
summary(Rainfall_BR_processed) #the summary function counts NAs 
...
ggplot(Rainfall_BR_processed, aes(x = Date, y = Rainfall)) +
  geom_line() + #use the lines not the scatterplot
  geom_smooth() +
...
ggplot(Temp_BR_processed, aes(x = Date, y = Temperature_C)) +
  geom_line() +
  geom_smooth() +
  labs(x = "", y = expression("Temperature (Celsius)"))


```
Just by looking at the visualization, it appears that the temperature set have some kind of trend
...
f_month_rainfall <- month(first(Rainfall_BR_processed$Date)) #first() calls the first observation of the column
f_year_rainfall <- year(first(Rainfall_BR_processed$Date)) #same thing, but first year not-first month
rainfall_ts <- ts(Rainfall_BR_processed$Rainfall, #remember to call the column we want to convert to a time series object
                   start=c(f_year_rainfall,f_month_rainfall), #notice we use the f_month and f_year here to start off the time series 
                   frequency=12) #12 = monthly data

f_month_temperature <- month(first(Temp_BR_processed$Date)) #first() calls the first observation of the column
f_year_temperature <- year(first(Temp_BR_processed$Date)) #same thing, but first year not-first month
temperature_ts <- ts(Temp_BR_processed$Temperature_C,
                   start=c(f_year_temperature,f_month_temperature), #notice we use the f_month and f_year here to start off the time series 
                   frequency=12) #12 = monthly data
...
rainfall_decomp <- stl(rainfall_ts,s.window = "periodic") #when do we choose something that's not periodic
plot(rainfall_decomp)

temperature_decomp <- stl(temperature_ts,s.window = "periodic") #when do we choose something that's not periodic
...
rainfall_components <- as.data.frame(rainfall_decomp$time.series[,1:3]) #$time.series[,1:3] is how you call the 3 decomposed plots
rainfall_components


temperature_components <- as.data.frame(temperature_decomp$time.series[,1:3]) #$time.series[,1:3] is how you call the 3 decomposed plots
...
#heyyy we can see how the trend moves along with the whole data

ggplot(temperature_components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") +
  ylab(expression("Temperature (Celsius)"))
#heyyy we can see how the trend moves along with the whole data
...
temperature_trend <- Kendall::SeasonalMannKendall(temperature_ts)
...
# Exercise 12: Now remove the seasonality and apply another test. Compare the results with what you obtained on Exercise 9 (do you mean 11?).

rainfall_deseasoned_ts <- rainfall_ts - rainfall_decomp$time.series[,1]

temperature_deseasoned_ts <- temperature_ts - temperature_decomp$time.series[,1]


rainfall_trend2 <- Kendall::MannKendall(rainfall_deseasoned_ts)

rainfall_trend2
summary(rainfall_trend2)

temperature_trend2 <- Kendall::MannKendall(temperature_deseasoned_ts)
...
> Answer: There is without a doubt, a trend in both the temperature and rainfall data sets. This is confirmed by both a seasonal and deseasoned Mann Kendall test

```{r}
#just looking at the Mann Kendall tests for both the seasonal and de-seasoned data all at once
summary(rainfall_trend)
summary(rainfall_trend2)
summary(temperature_trend)
summary(temperature_trend2)
