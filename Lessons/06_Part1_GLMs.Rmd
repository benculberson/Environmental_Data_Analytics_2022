...
## Objectives
1. Describe the components of the generalized linear model (GLM) <-that's what this is about
...
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth) #y ~ x -> we're predicting y with x
summary(irradiance.regression)

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
#ok so we have some reasonabl relationship here, but there's more to the story
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

>  In this model, depth explains 30.91% of the irradiance in the water (statistically significant at the 0.01 level). It matters
...
Given the distribution of irradiance values, we don't have a linear relationship between x and y in this case. Let's try log-transforming the irradiance values. Note we are also removing the observations that seems to be outliers.
...
irradiancebydepth2 <- 
    ...
    geom_smooth(method = 'lm', se = FALSE, color = "black") #the se = FALSE thing removes the standard errors, but I dont even see one anyways, probably because it's so small
print(irradiancebydepth2)
...
step(TPAIC)
#the below model is the selection after seeing the AIC
#the TPAIC ends its output with its recommended models
#the final model is the optimal one
