...
GasPlants <- st_read('https://opendata.arcgis.com/datasets/a0722062315143a7980cebb79a204781_4.geojson') #st_read is the equivalent as read csv
#Alternative
...
#View using sf plot
plot(GasPlants['STATUS'],pch=16) #status is either active or inactive 

#View using ggplot
ggplot(data=GasPlants) + geom_sf()

#View using mapview
mapview(GasPlants) #so cool, you can see the NC overlay
...
EPAair.sites <- EPAair %>% 
  ...
  summarize(
    mean_Ozone = mean(Ozone, na.rm=T),    #What does "na.rm=T" do? Is it necessary? I think T means True, I don't think it's necessary because the mean is just going to ignore them
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), #note that long = x, and y = lat.
           crs=4326) #create the simple feature data frame
...
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) # i guess the crs puts it on a map, this is sort of a better data frame
...
mapview(sf_GraingerHall) #same view, now a spatial dataframe, this is the superior of the 3 dfs
...
European Petroleum Survey Group database point
...
#THis takes the difference between the two and converts it from degrees (Lat. Long coordinates to meters)
