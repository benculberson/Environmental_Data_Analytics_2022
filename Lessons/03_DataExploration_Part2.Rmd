...
class(USGS.flow.data$datetime) #we're checking to make sure the dates are dates
#after running this, it appears that the dates are class "character"

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y-%m-%d") 
class(USGS.flow.data$datetime) #we want a "date" now
...
```{r, fig.height = 3, fig.width = 4}
#Note the data is above the aesthetics
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.max))

#Note the changed bin width
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#Note the changed number of bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#This keeps the x axis from going beyond 500 (the scale_x_continuous)
...
```{r, fig.height = 3, fig.width = 4}
#This is a stacked set of data visualization
ggplot(USGS.flow.data) +
...
#In this one, we're literally coloring the value based on the categorical data in the other variable
ggplot(USGS.flow.data) +
...
#Taking different slices of the data and looking at the distribution, like a histogram of box plots
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#Shows the range of values, and the distribution within those ranges, we can see the same IQR and median but with a width that corresponds to frequency
ggplot(USGS.flow.data) +
...
> Answer: Bar-categorical, Histogram, frequency polygon, box and whisker, violin, scatterplot-numerical, but each one shows a different way of illustrating. Perhaps one of each? Depends on the data.

## Ending discussion

What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: THe distribution of the discharge was on display today, I could really see the different ways the water flowed over the course of the data set.


How can multiple options for data exploration inform our understanding of our data?

> Answer: Each illustration highlights different aspects of the data. No one graph ignores things (to an extent), but each graph brings certain components of the data to the forefront.
...
> Answer: As discussed in the video, the gage height has an interestin polinomial relationsip with the discharge of the river. It appears that as the volume increased, the gage height was slightly slower to respond. Makes sense-the river expands in width a little bit as the waters rise, resulting in a nonlinear relationship.
