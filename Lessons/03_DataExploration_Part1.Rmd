...
A session in RStudio will always function by mapping to a specific folder in your computer, called the *working directory*. All navigation between folders and files will happen relative to this working directory. When you open an R project, your working directory will automatically set to the folder that holds the project file. If you open an R script or RMarkdown document directly by double-clicking the file, you're working directory will automatically set to the folder that holds that file. It is a good idea to note with a comment at the top of your file which working directory you intend the user to designate.
...
> Answer: I get an attaching packages indicator for 6 packages and a conflicts indicator (specifically for dplyr). It does appear that everything has worked however.
...
#USGS.flow.data <- read.csv("../Data/Raw/USGS_Site02085000_Flow_Raw.csv", stringAsFactors = TRUE) <- will need this for assignment 3
#This will import the data as a vector and not a character!!! SUPER IMPORTANT
#This used to be the default, that is no longer the case and we must adapt
...
> ANSWER: I can find this file in the metadata folder. This is in a folder that describes the data in my raw data folder. This file also describes how to reproduce the data again if we so wished, ideal for any individual trying to reproduce our study using the data from the USGS site.

```{r exploredata}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data) #we want to check to make sure this is a dataframe
colnames(USGS.flow.data)

# Rename columns
#the c function is short for columnbind which takes a vector of data
...
class(USGS.flow.data$datetime)#checking the class of these columns
...
> Answer: They were recorded as NAs
...
```{r findNA}
summary(USGS.flow.data$discharge.mean) #shows the number of NAs in the data set
...
> Answer: If we have a time series research question and the NAs are at the beginning of the data, we may want to remove the NAs. If the NAs are in the middle of the data set, we may want to retain the NAs.

```{r removeNA}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #this removes the NAs (there are a lot)
dim(USGS.flow.data)
...
R will often import dates as factors or characters rather than dates. To fix, this we need to tell R that it is looking at dates. We also need to specify the format the dates are in. By default, if you don't provide a format, R will attempt to use %Y-%m-%d or %Y/%m/%d (capitalization makes a huge difference) as a default. Note: if you are working collaboratively in an international setting, using a year-month-day format in spreadsheets is the least ambiguous of date formats. Make sure to check whether month-day-year or day-month-year is used in an ambiguously formatted spreadsheet.
...
Note that for every date prior to 1969 (CRAP), R has assigned the date in the 2000s rather than the 1900s. This can be fixed with an `ifelse` statement inside a function. Run through the code below and write what is happening in the comment above each line.

```{r fixformartDate}
# 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")
#the format function puts the data back into a character 2027-10-01 is now 271001
#
...
#this function replaces the column with the function that we've created
#
