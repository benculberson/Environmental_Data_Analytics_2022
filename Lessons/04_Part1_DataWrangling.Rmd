...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #remember this is how we get our dates to not be lame
...
NTL.phys.data_edited2 <- NTL.phys.data[NTL.phys.data$year4 == "1999",]
NTL.phys.data_edited <- filter(NTL.phys.data, year4 == "1999")
NTL.phys.data_edited
NTL.phys.data_edited2

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data_edited3 <- NTL.phys.data[NTL.phys.data$year4 > 1989,]
NTL.phys.data_edited3 <- NTL.phys.data_edited3[NTL.phys.data_edited3$year4 < 2000,]
NTL.phys.data_edited3

NTL.phys.data_edited4 <- filter(NTL.phys.data, year4 > 1989 & year4 < 2000)
NTL.phys.data_edited4 
```
Question: Why don't we filter using row numbers?

> Answer: Because we don't want to have to hunt down which row number the period we're looking for begins and ends at.
...
NTL.phys.data.temperature.descending
```
### Select
...
#notice the mutate function does some good work here
...
#the second way of doing this is the better way, notice how you don't have to know the last day
```
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #get it down to those two lakes
  select(lakename, sampledate:temperature_C) %>% #select [lakename,sampledate:temperature_C] indxing by row, the column
  mutate(temperature_F = (temperature_C*9/5) + 32) #create a new column
...
So that if we fuck it up, we can go back and change it. Or at least, restart it.
